...
- **Покрытие кода** — 92.86% (cargo-tarpaulin), отчёт в `tarpaulin-report.html`
...
HTML-документация (cargo doc) находится в `doc/`. Для просмотра:

```bash
# Из сгенерированных файлов в проекте:
open doc/fibonacci_stable/index.html
...
doc/                        — HTML-документация (cargo doc)
tarpaulin-report.html       — отчёт покрытия тестами
```
